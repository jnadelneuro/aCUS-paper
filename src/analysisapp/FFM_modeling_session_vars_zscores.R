...
# === Patch for fastFMM parallel bug ===
patch_fastFMM_parallel <- function() {
  message("Applying hotfix to fastFMM::fui for parallel execution...")
  
  # 1. Get the current function body as text
  # width.cutoff=500 prevents line wrapping that might break the search pattern
  fui_fun <- fastFMM::fui
  fui_src <- deparse(body(fui_fun), width.cutoff = 500)
  
  # 2. Search for the specific string literal "RHat"
  # In the source, this appears inside the clusterExport varlist: c(..., "RHat")
  # We want to change it to: c(..., "RHat", "randint_flag")
  if (!any(grepl('"RHat"', fui_src))) {
    warning("Patch Failed: Could not find 'RHat' string in fui() body. Version might differ.")
    return()
  }
  
  # 3. Perform the substitution
  # This adds "randint_flag" to the list of exported variables
  fui_src_patched <- gsub('"RHat"', '"RHat", "randint_flag"', fui_src)
  
  # 4. Reconstruct and overwrite the function in the package namespace
  body(fui_fun) <- parse(text = paste(fui_src_patched, collapse = "\n"))[[1]]
  assignInNamespace("fui", fui_fun, ns = "fastFMM")
  
  message("Success: fastFMM::fui has been patched. 'randint_flag' will now be exported to workers.")
}
...
photoDF <- arrow::read_feather('photoDF_R_with_rates.feather')
names(photoDF)[names(photoDF) == "rewardRate"] <- "reward_rate"
target_event <- 'ReNP'
...
colnames <- c('pokeRate', 'entryRate', 'dayOnType', 'reward_rate', 'numShocks')
...
# Rename any files in the directory that contain "FFM" to use "FMM" instead.
if (dir.exists(rds_directory)) {
  all_files <- list.files(path = rds_directory, pattern = "FFM", full.names = TRUE)
  for (old_path in all_files) {
    new_path <- file.path(dirname(old_path), gsub("FFM", "FMM", basename(old_path)))
    if (old_path != new_path) {
      if (file.exists(new_path)) {
        warning(paste("Skipping rename; target already exists:", new_path))
      } else {
